    ...
    if (active) {
      schema = schema.test({
        test(value) {
          ...
          const { values, matchAll = true } =
            parseReference<IDoesNotContainProps>(this, props);

          const blacklist: (string | RegExp)[] =
            typeof values === "string" || values instanceof RegExp
              ? [values]
              : values;
          const blacklistString = blacklist.map((e) => e.toString());

          const result = blacklist[matchAll ? "every" : "some"](
            (e) => !check(value, e)
          );
